...
class SignUpViewModel @Inject constructor(
    ...
    val userData: StateFlow<User?> = _userData // can be accessed from UI (hold all user info for specific userId)
            ...
            if (success) {
                authRepository.getCurrentUserId()?.let { userId -> // if successful, gets the current user's user id
                    firestoreRepository.saveUserData(userId, email) { saveSuccess -> // and saves the user data in firestore
                        if (saveSuccess) {
                            fetchUserData(userId) // if successful, fetches the data of the current user
